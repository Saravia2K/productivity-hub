      ...
      <CardContent className="pt-5">
        <div className="flex items-start justify-between">
          <div className="space-y-2">
            ...
            <p className="text-3xl font-bold text-[var(--sea-ink)]">{value}</p>
            {trend && (
  ...
  return (
    <div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid gap-4 lg:grid-cols-3">
            ...
            <CardHeader className="p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-[var(--sea-ink-soft)]">Últimos 6 meses</p>
                </div>
          ...
          <Card>
            <CardHeader className="p-5">
              ...
              <p className="text-xs text-[var(--sea-ink-soft)]">Distribución actual</p>
            </CardHeader>
              ...
              <ul className="mt-2 space-y-1.5">
                {data.objectivesByStatus.map((entry) => (
                  <li key={entry.status} className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-semibold text-[var(--sea-ink)]">{entry.count}</span>
                  </li>
                ))}
              </ul>
            </CardContent>
            ...
            <CardHeader className="p-5">
              <div className="flex items-center justify-between">
                ...
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[rgba(79,184,178,0.12)]">
                  <Zap className="h-3 w-3 text-[var(--lagoon-deep)]" />
                </span>
              </div>
            </CardHeader>
                ...
                {data.recentActivity.map((item) => (
                  <li key={item.id} className="flex items-start gap-3 py-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm text-[var(--sea-ink)] leading-snug">{item.message}</p>
            ...
            <CardContent className="pt-2 space-y-2">
              {[
                ...
                <Link
                  key={action.to}
                  to={action.to}
                  className="flex items-center gap-3 rounded-xl border border-[var(--line)] p-3 transition-all hover:border-[var(--lagoon-deep)] hover:bg-[rgba(79,184,178,0.04)]"
                >
                  <div
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-[var(--sea-ink)]">{action.label}</p>
                    <p className="text-xs text-[var(--sea-ink-soft)]">{action.desc}</p>
                  </div>
                  <ArrowRight className="h-4 w-4 shrink-0 text-[var(--sea-ink-soft)]" />
                </Link>
              ))}
            </CardContent>
          </Card>
        ...
        {/* Satisfaction banner */}
        <Card className="bg-gradient-to-r from-[var(--sea-ink)] to-[#1a4550] border-0">
        ...
        {user && (
          <div className="flex items-center gap-2">
            <span className="text-xs text-[var(--sea-ink-soft)]">Tu rol:</span>
            <Badge variant={user.role}>{user.role}</Badge>
            {user.department && (
              <>
                <span className="text-xs text-[var(--sea-ink-soft)]">·</span>
                <span className="text-xs text-[var(--sea-ink-soft)]">{user.department}</span>
              </>
            )}
          </div>
        )}
